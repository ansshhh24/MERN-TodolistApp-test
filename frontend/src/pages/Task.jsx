  ...
  useEffect(() => {
    if (mode === "update") {
      ...
      fetchData(config, { showSuccessToast: false }).then((data) => {
        setTask(data.task);
        console.log("Original dueDate:", data.task.dueDate);
        setFormData({ 
          title: data.task.title,
          description: data.task.description,
          dueDate: data.task.dueDate ? new Date(data.task.dueDate).toLocaleDateString() : "",
        });
      });
    }
        ...
        <form className='m-auto my-16 max-w-[1000px] bg-white p-8 border-2 shadow-md rounded-md'>
          {loading ? (
            ...
            <>
              ...
              <Input type="text" name="title" id="title" value={formData.title} placeholder="Task Title" onChange={handleChange} />
              {fieldError("title")}
            </div>
              ...
              <Input type="date" name="dueDate" id="dueDate" value={formData.dueDate.slice(0, 10)} onChange={handleChange} />
              {fieldError("dueDate")}
            </div>
              ...
              <button className='ml-4 bg-primary-dark text-white px-4 py-2 font-medium hover:bg-red-600' onClick={() => navigate("/")}>Cancel</button>
